...
public class SpeedShootScenario extends AbstractScenario {
    ...
    public SpeedShootScenario(ScenarioSwitcher sw, SpeedShootScenario ts) {
        this(sw);
    ...
    public SCORE_TYPE getScoreType() {
        ...
        case 6:
            return SCORE_TYPE.SCORE_SPEED_SHOOT_6;
        case 5:
            return SCORE_TYPE.SCORE_SPEED_SHOOT_5;
        case 4:
            return SCORE_TYPE.SCORE_SPEED_SHOOT_4;
        default:
    ...
    protected void drawFrameInit(Graphics2D g2) {
        ...
        success = writeLine(DEV_SINGLE, CMD_SSHOT + " " + Integer.valueOf(getNumberOfTargets()).toString());
        if (!success) {
            return;
        }
    ...
    protected void drawFrameFinish(Graphics2D g2) {
        getSwitcher().setNextScenario(new SpeedShootScenario(getSwitcher(), this));
    }
    ...
    public synchronized void finishScenario() {
        ...
        for (int i = 0; i < sensors.size(); i++) {
            ...
            if (sen.getTimeHit() == 0) {
                tl.setText(String.format("T%d:penalty (+%2d.%03d)",
                        i + 1, PENALTY_MS / 1000, PENALTY_MS % 1000));
                penalty += PENALTY_MS;
            } else {
        ...
        printInfo(String.format("Total  : %d", before), null);
        printInfo(String.format("Penalty: %d", penalty), null);

        before = Long.min(TIMEOUT_MS, before + penalty);
    ...
    protected class BTDeviceHandler extends BTCommandHandler {
        private SpeedShootScenario scenario;

        public BTDeviceHandler(SpeedShootScenario s) {
            super(s);
        ...
        @Override
        public void cmdSshot(StringTokenizer st, int devid) {
            String next = st.nextToken();

            if (next.equalsIgnoreCase("OK")) {
                scenario.resetTimeStart();
            } else {
                scenario.printError(CMD_SSHOT + ": Command is failed.", null);
            }
        }
    ...
    protected class MouseHandler extends MouseAdapterEx {
        ...
        public MouseHandler(SpeedShootScenario s) {
            scenario = s;
        }
